...
const CategoryList = ({
    ...
    return (
        <div className="sticky top-5 pl-6 flex flex-col gap-6">
            ...
            <ul
                className={`${screenWidth >= 450 && "[mask-image:linear-gradient(to_top,transparent,black_60%)] [-webkit-mask-image:linear-gradient(to_top,transparent,black_90%)] min-[580px]:[mask-image:linear-gradient(to_top,transparent,black_40%)]"} flex flex-col gap-2 max-h-[75vh] overflow-y-scroll pb-20 `}
            >
                {categories.map((cat, index) => (
                    <li
                        key={index}
                        className={`${cat === selectedCategory && "font-bold"} cursor-pointer list-none flex flex-row gap-2 items-center`}
